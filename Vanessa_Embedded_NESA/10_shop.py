...
overlays = [
    cv2.imread(f"Glasses/glass{i}.png", cv2.IMREAD_UNCHANGED) for i in range(1, 34)
]
...
bluetooth_port = "COM10"  # Replace with your Bluetooth COM port
...
while True:
    ...
    if not face_recognized:
        ...
        for x, y, w, h in faces:
            # Recognize the face
            customer_uid, confidence = faceRecognizer.predict(
                gray[y : y + h, x : x + w]
            )
            ...
            if 60 < confidence < 85:
                try:
                    ...
                    c = conn.cursor()
                    c.execute(
                        "SELECT customer_name FROM customers WHERE customer_uid = ?",
                        (customer_uid,),
                    )
                    row = c.fetchone()
                ...
                if row:
                    ...
                    if recognition_count[customer_uid] >= REQUIRED_RECOGNITION_COUNT:
                        ...
                        if bluetooth:
                            ...
                            bluetooth.write(
                                f"{c_name} Recognized Successfully\r\n".encode()
                            )
                        break
        ...
        # Detect faces
        faces = faceCascade.detectMultiScale(
            gray_scale, scaleFactor=1.1, minNeighbors=5, minSize=(30, 30)
        )
        ...
        sign_image_resized = cv2.resize(
            sign_image, (224, 224), interpolation=cv2.INTER_AREA
        )
        sign_image_array = np.asarray(sign_image_resized, dtype=np.float32).reshape(
            1, 224, 224, 3
        )
        ...
        cv2.putText(
            sign_image,
            f"Sign: {class_name[2:]} ({confidence_score*100:.2f}%)",
            (10, 30),
            fontFace,
            1,
            (0, 255, 0),
        ...
        if (
            class_name[2:].lower() == "ok_sign" and confidence_score > 0.75
            ...
            if ok_sign_count >= REQUIRED_OK_SIGN_COUNT:
                try:
                    ...
                    c.execute(
                        "SELECT quantity FROM cart WHERE customer_uid = ? AND product_id = ?",
                        (current_customer_uid, num),
                    )
                    row = c.fetchone()
                    ...
                    if row:
                        ...
                        c.execute(
                            "UPDATE cart SET quantity = ? WHERE customer_uid = ? AND product_id = ?",
                            (new_quantity, current_customer_uid, num),
                        )
                    else:
                        # Insert a new record if the sunglasses are not in the cart
                        print(f"Inserting the sunglasses {num} in cart")
                        c.execute(
                            "INSERT INTO cart (customer_uid, product_id, quantity) VALUES (?, ?, ?)",
                            (current_customer_uid, num, 1),
                        )
                        c.execute(
                            "UPDATE customers SET confirm = 1 WHERE customer_uid = ?",
                            (current_customer_uid,),
                        )
                        conn.commit()
                        print(
                            f"{customer_name} confirmed and sunglasses updated in cart"
                        )
                ...
                if bluetooth:
                    ...
                    bluetooth.write(
                        b"Successful added glasses to the cart\r\n"
                    )  # Adjust message as needed
                    bluetooth.write(f'Successful added glasses {num} to the cart\r\n'.encode())  # Adjust message as needed
        ...
        cv2.putText(
            sign_image,
            f"OK Sign Counter: {ok_sign_count}/{REQUIRED_OK_SIGN_COUNT}",
            (10, 60),
            fontFace,
            1,
            (255, 0, 0),
